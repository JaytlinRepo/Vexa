...
;(function () {
  ...
  function rAudienceDeepDive(c) {
    return reportHeader(c.headline || 'Audience deep dive', c.dataSource ? (c.dataSource === 'stub' ? 'Based on typical patterns for your niche — connect your social account for insights tailored to you.' : 'Based on your connected account.') : null) +
  ...
  function rEngagementDiagnosis(c) {
    ...
    return reportHeader(c.headline || 'Engagement diagnosis', c.dataSource ? (c.dataSource === 'stub' ? 'Based on typical patterns for your niche — connect your social account for insights tailored to you.' : 'Based on your connected account.') : null) +
  ...
  function rSlotAudit(c) {
    ...
    return reportHeader(c.headline || 'Slot audit', c.dataSource === 'stub' ? 'Based on typical patterns for your niche — connect your account for data from your real posts.' : 'Based on your connected account.') +
      slots.map((s) => `
        <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:14px 16px;margin-bottom:10px">
